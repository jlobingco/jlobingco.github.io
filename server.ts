...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS users (
    ...
    role TEXT DEFAULT 'editor', -- 'admin', 'editor', 'member'
    name TEXT,
    member_id INTEGER,
    FOREIGN KEY(member_id) REFERENCES members(id)
  );
...
// Migration: Add 'member_id' column to 'users' if it doesn't exist
const userTableInfo = db.prepare("PRAGMA table_info(users)").all();
const hasMemberId = userTableInfo.some((col: any) => col.name === 'member_id');
if (!hasMemberId) {
  db.exec("ALTER TABLE users ADD COLUMN member_id INTEGER");
}

// Seed default admin if not exists
const adminExists = db.prepare("SELECT * FROM users WHERE email = 'admin@saversfund.com'").get();
if (!adminExists) {
  db.prepare("INSERT INTO users (email, password, role, name) VALUES (?, ?, ?, ?)")
    .run('admin@saversfund.com', 'admin123', 'admin', 'Juvelyn Lobingco');
}
...
async function startServer() {
  ...
  app.post("/api/auth/login", (req, res) => {
    ...
    if (user) {
      // In a real app, use JWT or sessions. For this prototype, we'll just return the user.
      res.json(user);
    } else {
  ...
  app.post("/api/users/invite", (req, res) => {
    const { email, password, role, name, member_id } = req.body;
    try {
      const result = db.prepare("INSERT INTO users (email, password, role, name, member_id) VALUES (?, ?, ?, ?, ?)")
        .run(email, password || 'password123', role || 'editor', name, member_id);
  ...
  app.get("/api/users", (req, res) => {
    const users = db.prepare("SELECT id, email, role, name, member_id FROM users").all();
    res.json(users);
  });
  ...
  app.get("/api/loans", (req, res) => {
    const loans = db.prepare(`
      ...
      CASE WHEN l.due_at < datetime('now') AND l.status = 'Active' THEN 1 ELSE 0 END as is_overdue
      FROM loans l
